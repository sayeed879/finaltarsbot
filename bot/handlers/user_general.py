"""
MENU, HELP, STATS, and General Text Handlers

These handlers are registered AFTER the FSM handlers (ai_chat, pdf_search)
to ensure they only run when the user is not in an active operation.
"""
...
async def handle_stats_command(message: Message, db_pool):
    ...
    if user.is_premium:
        ...
        ai_bar = "🟩" * (int(ai_percentage) // 10) + "⬜" * (10 - (int(ai_percentage) // 10))
        stats_text += (
    ...
    if user.is_premium:
        ...
        pdf_bar = "🟦" * (int(pdf_percentage) // 10) + "⬜" * (10 - (int(pdf_percentage) // 10))
        stats_text += (
...
# --- "About" and General Text trigger handler ---
ABOUT_TRIGGERS = [
    "who are you", 
    "who made you", 
    "developer", 
...
async def handle_general_text(message: Message, db_pool):
    ...
    # If no other trigger matched, it will fall through to the
    # unknown_text.py handler, which should be registered last.